//
//  AwardedNotificationTableViewController.m
//  Handychecks
//
...
@interface AwardedNotificationTableViewController ()

@end

@implementation AwardedNotificationTableViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *CellIdentifier = @"AwardedNotificationTableViewCell";
    
    AwardedNotificationTableViewCell *cell = (AwardedNotificationTableViewCell *)[tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if (cell == nil)
    {
        NSArray *nib;
        nib = [[NSBundle mainBundle] loadNibNamed:@"AwardedNotificationTableViewCell" owner:self options:nil];
    ...
    if([[dict objectForKey:@"job_status"] isEqualToString:@"accepted"]) {
        [cell.trackButton setTag:indexPath.row];
        [cell.trackButton addTarget:self action:@selector(track:) forControlEvents:UIControlEventTouchUpInside];
        cell.trackButton.layer.cornerRadius = 13.0f;
    } else {
        [cell.trackButton setHidden:YES];
    }
...
-(void)track:(UIButton *)sender {
    ...
    NSDictionary *urlDictionary = [NSDictionary dictionaryWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"Api" ofType:@"plist"]];
    NSString *url = [urlDictionary objectForKey:@"accepted_service_details"];
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
